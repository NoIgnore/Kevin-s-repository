...
void create(string pid, string parent, string child, int priority)
{
	...
	/*for (vector<pcb>::iterator it = all_process_list.begin(); it != all_process_list.end(); it++)
	{
		if (it->pid == a.pid && running_pid != "hhj" && running_pid != a.pid)
		{
			for (vector<pcb>::iterator it2 = all_process_list.begin(); it2 != all_process_list.end(); it2++)
			{
				if (it2->pid == running_pid)
				{
					it2->child = &(*it);
					it->parent = &(*it2);
					return;
